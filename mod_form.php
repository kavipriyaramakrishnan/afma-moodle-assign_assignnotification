...
class mod_assign_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        // Added by Pukunui - VFK-574585
        $mform->addElement('header', 'emailnotifications', 'Email '.get_string('notifications', 'assign'));
        $mform->addElement('text', 'thirdsubmission', get_string('thirdsubmission', 'assign'), array('size'=>'64'));
        if (!empty($assignid)) {
            $notifyemail = $DB->get_field('assign_notifyemail', 'notifyemail', array('assignid' => $assignid));
            $mform->setDefault('thirdsubmission', $notifyemail);
        }
        // End of changes by Pukunui - VFK-574585
