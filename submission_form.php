...
class mod_assign_submission_form extends moodleform {
    ...
    public function definition() {
        ...
        $sql = "SELECT max(attemptnumber) as maxattempt
                FROM {assign_submission}
                WHERE userid = $data->userid
                AND assignment = $instance->id";
        $attempt = $DB->get_record_sql($sql);
        if ($attempt->maxattempt >= 1 && $attempt->maxattempt < 2) {
            $mform->addElement('static', 'secondattemptmessage', get_string('secondattemptmessage', 'assign'));
        }
