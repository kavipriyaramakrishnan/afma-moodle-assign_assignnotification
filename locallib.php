...
class assign {
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        if ($submission->attemptnumber >= 2) {
            ...
            $notifyemail = $DB->get_field('assign_notifyemail', 'notifyemail', array('assignid' => $submission->assignment));
            if (!empty($notifyemail)) {
                $user = $DB->get_record('user', array('id' => $submission->userid));
                $assignname = $DB->get_record('assign', array('id' => $submission->assignment));
                $coursename = $DB->get_field('course', 'fullname', array('id' => $assignname->course));
                // From details.
                $from = new stdclass();
                $from->firstname = 'Learning';
                $from->lastname  = 'AFMA';
                $from->email     = 'noreply@learning.afma.com.au';

                // To details.
                $to->id   = 1;
                $to->firsname = 'AFMA Learning';
                $to->lastname = 'Administrator'; 
                $to->email    = $notifyemail;

                $a = new stdClass();
                $a->user = $user->firstname.' '.$user->lastname;
                $a->assign  = $assignname->name;
                $a->attemptnumber = ($submission->attemptnumber + 1);
                $a->coursename = $coursename; 
                $emailsub = get_string('notifyemailsub', 'assign');
                $emailbody = get_string('notifyemailbody' , 'assign', $a);
